...
My_Constrained_RDA_Plot_Type1_13 <- function(ccaConstrainedFort, ccaConstrained){
 ...
 coord_fixed(sqrt(ccaConstrained$CCA$eig[2]/ccaConstrained$CCA$eig[1])) +
  ...
  geom_segment(aes(x = 0, y = 0, xend  = RDA1 * 7, yend = RDA3 * 7), data = ccaConstrainedFort %>% filter(Score == "biplot"), inherit.aes = FALSE, size = 1, alpha = 0.9, arrow = arrow(length = unit(0.1, "in")), color = "black") +
  #geom_point(aes(x  = CCA1, y = CCA2), data = ccaConstrainedFort %>% filter(Score == "biplot"), inherit.aes = FALSE) +
    labs(x = paste0("RDA1 (", round(ccaConstrained$CCA$eig[1]/ccaConstrained$tot.chi, 3) * 100, "%)"),
...
My_Constrained_RDA_Plot_Type1_wFort_13(rdaConstrained)
```
...
anova(rdaConstrained_NoFree, by = "margin", permutations = how(nperm=9999))
```
```{r}
...
anova(rdaConstrained_NoSmall, by = "margin", permutations = how(nperm=9999))
```
```{r}
...
plot_grid(plotNoFreeRda, plotNoSmallRda, labels = LETTERS, nrow = 2)
```
