...
```{r}
anova(ccaConstrained, by = "margin")
```

```{r}
permutest(ccaConstrained)
```
...
ccaConstrainedPlot <- ccaConstrainedFort %>%
 ...
 geom_text(aes(label = Label, x = CCA1 * 2, y = CCA2 * 2), color = "black",
...
This should run vs lat, not station.
And names should be easier.
Done
...
My_Constrained_CCA_Plot(ccaConditionedAllSizes)
```
