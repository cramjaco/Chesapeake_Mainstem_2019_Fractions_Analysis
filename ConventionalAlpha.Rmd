...
```{r}
plot_pielouJ_pred <- diversityDataWB %>%
...
diversityDataWB$pred_break = predict(breakLm, se.fit = TRUE)$fit
diversityDataWB$se_break = predict(breakLm, se.fit = TRUE)$se.fit
```
```{r}
...
alphaSummary2 %>% flextable() %>% merge_v(j = 1) %>% theme_vanilla() %>% bold(i = ~ p< 0.05, j = "p") %>% colformat_md() %>% set_table_properties(layout = "autofit") %>%
  align(j = -c(1:2), align = "right")
```
