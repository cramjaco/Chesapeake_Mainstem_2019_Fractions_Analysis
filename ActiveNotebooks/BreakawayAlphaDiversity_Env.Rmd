...
myBet_Env_Depth <- betta(
  formula = estimate ~ log(Size_Class) + I(log(Size_Class) ^2) + Salinity +  I(Salinity^2) +Oxygen + depth,
  ses = error,
  data = richSummaryExpanded  %>% mutate(Oyxgen = log10(Oxygen + 0.03))
)
```
...
```{r}
myBet_Env <- betta(
  formula = estimate ~ log(Size_Class) + I(log(Size_Class) ^2) + Salinity +  I(Salinity^2) +Oxygen,
  ses = error,
...
```{r}
breakLm_Env <- lm(estimate ~ log(Size_Class) + I(log(Size_Class) ^2) + Salinity +  I(Salinity^2) + Oxygen + depth, data = richSummaryExpanded %>% mutate(Oxygen = log10(Oxygen + 0.03)))
summary(breakLm_Env)
```