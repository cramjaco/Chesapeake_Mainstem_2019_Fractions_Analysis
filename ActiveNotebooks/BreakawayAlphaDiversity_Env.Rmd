...
This is the same as BreakawayAlphDiversity, but this time, I'm using the enviornmental data framing (Salinity + Oxygen) rather than latitude and depth.
...
```{r}
myBet <- betta(
  formula = estimate ~ sqrt(Size_Class) + I(sqrt(Size_Class) ^2) + lat +  I(lat^2) + depth,
  ses = error,
...
```{r}
breakLm <- lm(estimate ~ sqrt(Size_Class) + I(sqrt(Size_Class) ^2) + lat +  I(lat^2) + depth, data = richSummaryExpanded)
summary(breakLm)
```
...
######################
# As above but with enviromental framing
##########################
...
```{r}
myBet_Env_Depth <- betta(
  formula = estimate ~ log10(Size_Class) + I(log10(Size_Class) ^2) + Salinity +  I(Salinity^2)  + log10(Oxygen + 0.03) + depth,
  ses = error,
  data = richSummaryExpanded
)
```

```{r}
myBet_Env_Depth$table
```


```{r}
myBet_Env <- betta(
  formula = estimate ~ log10(Size_Class) + I(log10(Size_Class) ^2) + Salinity +  I(Salinity^2) +log10(Oxygen + 0.03),
  ses = error,
  data = richSummaryExpanded
)
```
...
There is a statistical effect for everything except oxygen.
...
myBet_Env$table %>% as.data.frame() %>% rownames_to_column(var = "Parameter") %>%
  mutate(Estimates = format(Estimates, digits = 2, scientific = TRUE) %>%
           reformat_sci()
         ) %>%
  mutate(`Standard Errors` = format(`Standard Errors`, digits = 2, scientific = TRUE) %>%
  ...
  mutate(Parameter = str_replace(Parameter, "^I?\\((.*)\\)", "\\1"),
         Parameter = str_replace(Parameter, "\\^2", "\\^2\\^"),
         Parameter = str_replace(Parameter, "log10", "log\\~10\\~"),
         Parameter = str_replace(Parameter, "depth", "Depth"),
         Parameter = str_replace(Parameter, "lat", "Latitude"),
         Parameter = str_replace(Parameter, "_", " ")# BOOKMARK!!
         ) %>%
...
```{r}
breakLm_Env <- lm(estimate ~ log10(Size_Class) + I(log10(Size_Class) ^2) + Salinity +  I(Salinity^2) + log10(Oxygen + 0.03), data = richSummaryExpanded)
summary(breakLm_Env)
```