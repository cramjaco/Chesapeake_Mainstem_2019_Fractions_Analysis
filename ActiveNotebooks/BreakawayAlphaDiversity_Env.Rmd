...
myBet_Env_Depth <- betta(
  formula = estimate ~ sqrt(Size_Class) + I(sqrt(Size_Class) ^2) + Salinity +  I(Salinity^2)  + Oxygen + depth,
  ses = error,
  data = richSummaryExpanded  %>% mutate(Oxygen = log10(Oxygen + 0.03))
)
```
...
```{r}
myBet_Env <- betta(
  formula = estimate ~ sqrt(Size_Class) + I(sqrt(Size_Class) ^2) + Salinity +  I(Salinity^2) +Oxygen,
  ses = error,
  data = richSummaryExpanded %>% mutate(Oxygen = log10(Oxygen + 0.03))
)
```
...
```{r}
breakLm_Env <- lm(estimate ~ sqrt(Size_Class) + I(sqrt(Size_Class) ^2) + Salinity +  I(Salinity^2) + Oxygen, data = richSummaryExpanded %>% mutate(Oxygen = log10(Oxygen + 0.03)))
summary(breakLm_Env)
```