...
We just got data back from UC davis of carbon and nitrogne content of particles.
This file is for examining that data

I refer to things as "Molarity" a lot here, including in the title, because I originally though the measurments were in moles/L.  They are not, everything is in mg/L. The units in the figures should be correct.

2023 March 20 -- I'm updating the figure S2 -- MCN_Molarity.png to correct the units.
...
# This operation stopped sticking for some reaon, so I'm redoing it.
microbialAbundance <- microbialAbundance %>% 
...
molarityPlot0 <- plot_grid(
  ...
  cb_plotter(CarbonPerLiter_mg/Bin_Size, logify = TRUE, lab = "POC (mg/L/μm)"),
  cb_plotter(NitrogenPerLiter_mg/Bin_Size, logify = TRUE, lab = "PON (mg/L/μm)"),
  nrow = 1,
  labels = LETTERS
)
...
# Carbon to nitrogen ratios for all of the particles
So I can say how C:N vary across space
```{r}
microbialAbundance %>%
  select(Station, Size_Class, Depth, CarbonPerLiter_mg, NitrogenPerLiter_mg) %>%
  pivot_longer(cols = c(CarbonPerLiter_mg, NitrogenPerLiter_mg), names_to = "measurement", values_to = "mg") %>%
  filter(Size_Class>0.2) %>%
  #filter(is.na(mg)) %>%
  group_by(Station, Depth, measurement) %>%
  summarise(mg = sum(mg)) %>%
  pivot_wider(names_from = measurement, values_from = mg) %>%
  mutate(CtoN = CarbonPerLiter_mg/NitrogenPerLiter_mg) %>%
  identity()
```
