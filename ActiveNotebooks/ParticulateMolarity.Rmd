...
cb_plotter(CarbonPerLiter/MassperLiter, logify = FALSE, lab = "POC/Mass (μmol/mg)") + scale_y_log10()
cb_plotter(NitrogenPerLiter/MassperLiter, logify = FALSE, lab = "PON/Mass (μmol/mg)") + scale_y_log10()
cb_plotter(CarbonPerLiter/NitrogenPerLiter, logify = FALSE, lab = "POC/PON") + geom_hline(yintercept = 106/16)  + scale_y_log10()
...
molarityPlot0 <- plot_grid(
  cb_plotter(MassperLiter/Bin_Size, logify = TRUE, lab = "Particle Mass (mg/L/mm)"),
  cb_plotter(CarbonPerLiter/Bin_Size/1000, logify = TRUE, lab = "POC (mg/L/mm)"),
  cb_plotter(NitrogenPerLiter/Bin_Size/100, logify = TRUE, lab = "PON (mg/L/mm)"),
  nrow = 1,
  labels = LETTERS
)
...
```{r}
ratioPlot <- plot_grid(
  cb_plotter(CarbonPerLiter/MassperLiter/1000, logify = TRUE, lab = "POC/Mass") + scale_y_log10(),
  cb_plotter(NitrogenPerLiter/MassperLiter/1000, logify = TRUE, lab = "PON/Mass") + scale_y_log10(),
  cb_plotter(CarbonPerLiter/NitrogenPerLiter, logify = TRUE, lab = "POC/PON") + scale_y_log10() + geom_hline(yintercept = 106/16),
  nrow = 1,
  labels = LETTERS
) %>% append_xg()
...
# Primary figure
The goal here is to remake Figure 1 so it shows
Mass, Carbon:Mass
16S/L/mm 16S/mg partiles


Before I do that. Lets get carbon and nitrogen fractions
```{r}
# plot_grid(abunPlot_perL, abunPlot_perMg,
# cb_plotter(CarbonPerLiter/MassperLiter/1000, logify = FALSE, lab = "POC/Mass (mg/mg)") + scale_y_log10()
# )
```

Well, these numbers are greater than 1, which is a problem, since it implies more carbon and nitrogen mass than particle mass.
