...
Step 3 -- CCA with Size, factoring out Station and Depth, removing
Free-living microbes

Step 4 - CCA with Size, factoring out Station and Depth, \> 5 micron
only

Try to get nice looking CCA plots with symbols on each

# To do -- I wonder what happens if I log transform the size class instead of sqrt transforming
...
myPlotDetails <- list(
  ...
  scale_color_manual(values = c("green2", "blue", "black")) ,
...
```{r}
rdaConstrained <- rda(wideASVs_log ~ lat + Depth2 + log10(Size_Class), data = sampleData01)
```
...
Lets make this into a nice table Columns are variance?, percent variance
explained, F, P And a row added for residual and for total
...
betaTable <- mooAnova %>% broom::tidy() %>%
  ...
  mutate(term = recode(term, lat = "Latitude", Depth2 = "Depth", `log10(Size_Class)` = "√Size Class")) %>%
...
This looks pretty awful, but Its not going in the paper I don't think so
I'll leave it for now.

## Main CCA Doing and Plotting Functions
...
My_Constrained_RDA_Plot_Type1 <- function(ccaConstrainedFort, ccaConstrained){
  ccaConstrainedPlot <- ccaConstrainedFort %>%
    ...
    labs(x = paste0("RDA1 (", round(ccaConstrained$CCA$eig[1]/ccaConstrained$tot.chi, 3) * 100, "%)"),
         ...
         shape = "Station",
         size = "Size Class") +
        geom_segment(aes(x = 0, y = 0, xend  = RDA1 * 6, yend = RDA2 * 6), data = ccaConstrainedFort %>% filter(Score == "biplot"), inherit.aes = FALSE, size = 1, alpha = 0.9, arrow = arrow(length = unit(0.1, "in")), color = "gray20") +
    geom_text(aes(label = Label2, x = RDA1 * 7.5, y = RDA2 * 7.5),
               color = "black", alpha = 1,
               show.legend = FALSE,
               data = ccaConstrainedFort %>%
                 filter(Score == "biplot") %>%
                 mutate(Label = recode(Label, lat = "Lat",
                                       `log10(Size_Class)` = "Size",
                                       `Depth2Surface` = "Surface",
                                       `log10(Oxygen + 0.03)` = "Oxygen"
                 )) %>%
                 ...
                 mutate(RDA1 = if_else(sqrt(RDA1^2 + RDA2^2) < 0.5, RDA1 * 2, RDA1)) %>% # was 1.5
                 mutate(RDA2 = if_else(sqrt(RDA1^2 + RDA2^2) < 0.5, RDA2 * 2, RDA2)) %>%
                 identity()
    ) +

    lims(x = c(range(ccaConstrainedFort$RDA1)[1] * 1.1, range(ccaConstrainedFort$RDA1)[2] * 1.7), y = range(ccaConstrainedFort$RDA2 * 1.1)) + # BOOKMARK
    myPlotDetails +
...
ggsave(here::here("Figures", "mainRDA.svg"),mainRDA, height = 4.4, width = 6)
```

Axis 1 vs 3 Not the most dry code I've ever done, but I'm not sure how
to actually do this, so I'm manually changing the axes
...
My_Constrained_RDA_Plot_Type1_13 <- function(ccaConstrainedFort, ccaConstrained){
 ...
 coord_fixed(sqrt(ccaConstrained$CCA$eig[2]/ccaConstrained$CCA$eig[1])) +
  ...
  geom_segment(aes(x = 0, y = 0, xend  = RDA1 * 6, yend = RDA3 * 6), data = ccaConstrainedFort %>% filter(Score == "biplot"), inherit.aes = FALSE, size = 1, alpha = 0.9, arrow = arrow(length = unit(0.1, "in")), color = "gray20") +
  #geom_point(aes(x  = CCA1, y = CCA2), data = ccaConstrainedFort %>% filter(Score == "biplot"), inherit.aes = FALSE) +
    labs(x = paste0("RDA1 (", round(ccaConstrained$CCA$eig[1]/ccaConstrained$tot.chi, 3) * 100, "%)"),
       ...
       shape = "Station",
       size = "Size Class") +
     geom_text(aes(label = Label2, x = RDA1 * 7.5, y = RDA3 * 7.5),
            color = "black", alpha = 1,
            show.legend = FALSE,
           data = ccaConstrainedFort %>%
             filter(Score == "biplot") %>%
    ...
    lims(x = c(range(ccaConstrainedFort$RDA1)[1] * 1.1, range(ccaConstrainedFort$RDA1)[2] * 1.7), y = range(ccaConstrainedFort$RDA3 * 1.1)) +
  myPlotDetails +
  guides(color = guide_legend(override.aes = list(shape = 21, size = 2, stroke = 2, fill = "gray")),
         shape = guide_legend(override.aes = list(size = 3)),
...
ggsave(here::here("Figures", "mainRDA_axis1and3.svg"),mainRDA13, height = 4, width = 6)
```
...
EnvRDA123 <- plot_grid(
  plot_grid(
    ...
    EnvRDA13 + theme(legend.position = "none"),
    ncol = 1, labels = c("A", "B"), label_y = 0.9
  ),
  legend_rda, rel_widths = c(10, 2)
)
EnvRDA123

```{r fig.height=7, fig.width=6}
legend_rda <- get_legend(
  mainRDA + theme(legend.box.margin = margin(0, 0, 0, 0))
                         )

PosRDA123 <- plot_grid(
  plot_grid(
    mainRDA + theme(legend.position = "none"),
    mainRDA13 + theme(legend.position = "none"),
    ncol = 1, labels = c("A", "B"), label_y = 0.9
  ),
  legend_rda, rel_widths = c(10, 2)
)
...
```
```{r}
ggsave(here("Figures", "PosRDA_123.png"), plot = PosRDA123, height = 7, width = 6)
ggsave(here("Figures", "PosRDA_123.svg"), plot = PosRDA123, height = 7, width = 6)
```
...
```{r}
rdaConstrained_NoFree <- rda(wideASVs_log_NoFree ~ lat + Depth2 + log10(Size_Class), data = sampleData_NoFree)
```
...
plotNoFreeRda <- My_Constrained_RDA_Plot_Type1_wFort(rdaConstrained_NoFree) + 
  scale_size(breaks = sqrt(filterSizes),
             labels = filterSizes, range = c(1.5,6))
...
```{r}
rdaConstrained_NoSmall <- rda(wideASVs_log_NoSmall ~ lat + Depth2 + log10(Size_Class), data = sampleData_NoSmall)
```
...
```{r}
plotNoSmallRda<- My_Constrained_RDA_Plot_Type1_wFort(rdaConstrained_NoSmall) + lims(x = c(-10, 11)) + scale_size(breaks = sqrt(filterSizes),
             labels = filterSizes, range = c(2.5,6))
plotNoSmallRda
```

```{r fig.width=7, fig.height = 8}
plot_grid(plotNoFreeRda ,
                           plotNoSmallRda ,
                           labels = LETTERS, nrow = 2)
```

Cowplot the last two together so they can be one supplemental figure \>
These looked different from eachother. Check them.
...
bigSubsetsRda0 <- plot_grid(plotNoFreeRda + theme(legend.position = "none"),
                           ...
                           labels = LETTERS, nrow = 2)
bigSubsetsRda <- plot_grid(bigSubsetsRda0, legend_rda, rel_widths = c(10, 2))
bigSubsetsRda
```

```{r}
ggsave(here::here("Figures", "bigSubsetsRda.png"), bigSubsetsRda, height = 8, width = 8)
ggsave(here::here("Figures", "bigSubsetsRda.svg"), bigSubsetsRda, height = 8, width = 8)
```
...
```{r}
rdaConstrained_env <- rda(wideASVs_log ~ Salinity + Depth2 + log10(Size_Class), data = sampleData01)
```
...
```{r}
rdaConstrained_wdepth <- rda(wideASVs_log ~ Salinity + Depth_m  + log(Oxygen + 0.03) + log10(Size_Class), data = sampleData01)
anova(rdaConstrained_wdepth, by = "margin")
```

```{r}
rdaConstrained_env3 <- rda(wideASVs_log ~ Salinity  + Oxygen + Depth_m + log10(Size_Class), data = sampleData01 %>% mutate(Oxygen = log(Oxygen + 0.03)))
mooAnova_Env3 <- anova(rdaConstrained_env3, by = "margin", permutations = 9999)
mooAnova_Env3
```

```{r}
rdaConstrained_env2 <- rda(wideASVs_log ~ Salinity  + Oxygen + log10(Size_Class), data = sampleData01 %>% mutate(Oxygen = log(Oxygen + 0.03)))
mooAnova_Env <- anova(rdaConstrained_env2, by = "margin", permutations = 9999)
mooAnova_Env
```
...
EnvRDA
EnvRDA13 <- My_Constrained_RDA_Plot_Type1_wFort_13(rdaConstrained_env2)
```
...
```{r}
ggsave(here("Figures", "EnvRDA_12.png"), plot = EnvRDA, height = 4.5, width = 6)
ggsave(here("Figures", "EnvRDA_13.png"), plot = EnvRDA13, height = 5, width = 6)
ggsave(here("Figures", "EnvRDA_12.svg"), plot = EnvRDA, height = 4.5, width = 6)
ggsave(here("Figures", "EnvRDA_13.svg"), plot = EnvRDA13, height = 5, width = 6)
```


```{r}
ggsave(here("Figures", "EnvRDA_123.png"), plot = EnvRDA123, height = 7, width = 6)
ggsave(here("Figures", "EnvRDA_123.svg"), plot = EnvRDA123, height = 7, width = 6)
```
...
sampleData_NoSmall \<- sampleData01 %\>% filter(Size_Class \> 2)
wideASVs_log_NoSmall \<- wideASVs_log[rownames(wideASVs_log) %in%
sampleData_NoSmall$ID,] wideASVs_log_NoSmall <- wideASVs_log_NoSmall[match(sampleData_NoSmall$ID,
rownames(wideASVs_log_NoSmall)),]

Particle characteristics? Have to strike the 0.2 and omit any missing
values.
...
```{r}
rdaConstrained_env_particles <- rda(wideASVs_log_MeasuredParticles ~ Salinity  + log(Oxygen + 0.03) + log10(Size_Class) + I(CarbonPerLiter_mg/MassperLiter) + I(NitrogenPerLiter_mg/CarbonPerLiter_mg) + `δ13CVPDB (‰)` + `δ15NAir (‰)`, data = sampleData_MeasuredParticles)
```

```{r}
extractAIC(rdaConstrained_env_particles)
```
...
# No Free and Small, but this time with an environmental focus

rdaConstrained_env2 \<- rda(wideASVs_log \~ Salinity + Oxygen +
log10(Size_Class), data = sampleData01 %\>% mutate(Oxygen = log(Oxygen +
0.03)))

```{r}
rdaConstrained_NoFree_Env <- rda(wideASVs_log_NoFree ~ Salinity + Oxygen + log10(Size_Class), data = sampleData_NoFree %>% mutate(Oxygen = log(Oxygen + 0.03)))
```

```{r}
rdaConstrained_NoFree_Env
anovaNofree_Env <- anova(rdaConstrained_NoFree_Env, by = "margin", permutations = how(nperm=9999))
anovaNofree_Env
```

```{r}
plotNoFreeRda_Env <- My_Constrained_RDA_Plot_Type1_wFort(rdaConstrained_NoFree_Env) + 
  scale_size(breaks = sqrt(filterSizes),
             labels = filterSizes, range = c(1.5,6))
...
```{r}
rdaConstrained_NoSmall_Env <- rda(wideASVs_log_NoSmall ~ Salinity + Oxygen + log10(Size_Class), data = sampleData_NoSmall %>% mutate(Oxygen = log(Oxygen + 0.03)))
```

```{r}
rdaConstrained_NoSmall_Env
anovaNoSmall_Env <- anova(rdaConstrained_NoSmall_Env, by = "margin", permutations = how(nperm=9999))
anovaNoSmall_Env
```

```{r}
plotNoSmallRda_Env<- My_Constrained_RDA_Plot_Type1_wFort(rdaConstrained_NoSmall_Env) + lims(x = c(-10, 11)) + scale_size(breaks = sqrt(filterSizes),
             labels = filterSizes, range = c(2.5,6))
plotNoSmallRda_Env
```

```{r fig.width=7, fig.height = 8}
plot_grid(plotNoFreeRda_Env ,
                           plotNoSmallRda_Env ,
                           labels = LETTERS, nrow = 2)
```
...
```{r fig.width = 7, fig.height = 8}
legend_rda <- get_legend(
  plotNoFreeRda_Env + theme(legend.box.margin = margin(0, 0, 0, 0))
                         )

bigSubsetsRda0_Env <- plot_grid(plotNoFreeRda_Env + theme(legend.position = "none"),
                           plotNoSmallRda_Env + theme(legend.position = "none"),
                           labels = LETTERS, nrow = 2)
bigSubsetsRda_Env <- plot_grid(bigSubsetsRda0_Env, legend_rda, rel_widths = c(10, 2))
bigSubsetsRda_Env
```

```{r}
ggsave(here::here("Figures", "bigSubsetsRda_Env.png"), bigSubsetsRda_Env, height = 8, width = 8)
ggsave(here::here("Figures", "bigSubsetsRda_Env.svg"), bigSubsetsRda_Env, height = 8, width = 8)
```

# Tables with Enviornmental Frame

```{r}
threeAnovas_Env <- bind_rows(
mooAnova_Env %>% broom::tidy() %>% mutate(dataset = "All"),
anovaNofree_Env %>% broom::tidy() %>% mutate(dataset = "Particle Associated"),
anovaNoSmall_Env %>% broom::tidy() %>% mutate(dataset = "≥ 5 Micron Particles")
) %>%
  select(Dataset = dataset, everything())

betaThreeTable_Env <- threeAnovas_Env %>%
  group_by(Dataset) %>%
  #mutate(`% Variance` = paste0(round(Variance/sum(Variance) * 100, 1), "%")) %>%
  mutate(`% Variance` = scales::percent(Variance/sum(Variance), accuracy = 0.1)) %>%
  
  mutate(term = recode(term, lat = "Latitude", Depth2 = "Depth", `log10(Size_Class)` = "log10(Size Class)")) %>%
  select(Dataset, Term = term, DF = df, Variance, `% Variance`, T = statistic, p = p.value) %>%
    mutate(Variance = format(Variance, digits = 3),
         `% Variance` = format(`% Variance`, digits = 2),
         `T` = if_else(is.na(`T`), "", format(`T`, digits = 2)),
...
betaThreeTable_Env %>% save_as_docx(path = here::here("Tables", "betaThreeTable_Env.docx"))
betaThreeTable_Env
```

These percent variance numbers aren't quite matching the figures. For
instance, "size" explains less variance in the table than the figures.
Need to investegate.
