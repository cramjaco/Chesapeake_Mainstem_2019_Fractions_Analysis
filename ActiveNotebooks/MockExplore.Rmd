...
Why are there doubles of each ASV? (Answer -- problems with inner join function, now resolved)
...
```{r}
counts_long_mock_01 %>% filter(SubType == "Even") %>% mutate(RAOver1Pct = RA > 0.01) %>% group_by(Run, RAOver1Pct)  %>% summarise(min = min(RA), max = max(RA), sum = sum(RA), n = n()) %>% arrange(RAOver1Pct)
```
