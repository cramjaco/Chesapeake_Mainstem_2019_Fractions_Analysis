...
```{r}
obsMod <- lm(S.obs ~ log(Size_Class) + I(log(Size_Class)^2)  + Salinity + I(Salinity^2) + Oxygen, data = diversityData %>% mutate(Oxygen = log10(Oxygen + 0.03)))
summary(obsMod)
```
...
```{r}
chao1Mod <- lm(S.chao1 ~ log(Size_Class) + I(log(Size_Class)^2) + I(log(Size_Class)^2) + Salinity + I(Salinity^2) + Oxygen, data = diversityData %>% mutate(Oxygen = log10(Oxygen + 0.03)))
summary(chao1Mod)
```
...
```{r}
shanMod <- lm(shannonH ~ log(Size_Class) + I(log(Size_Class)^2) + Salinity + I(Salinity^2) + Oxygen, data = diversityData %>% mutate(Oxygen = log10(Oxygen + 0.03)))
```
...
```{r}
pielouMod <- lm(pielouJ ~ log(Size_Class) + I(log(Size_Class)^2) + Salinity + I(Salinity^2) + Oxygen, data = diversityData %>% mutate(Oxygen = log10(Oxygen + 0.03)))
summary(pielouMod)
```
