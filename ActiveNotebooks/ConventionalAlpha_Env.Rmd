...
```{r}
obsMod <- lm(S.obs ~ log(Size_Class) + I(log(Size_Class)^2)  + Salinity + I(Salinity^2) + Oxygen + depth, data = diversityData %>% mutate(Oxygen = log10(Oxygen + 0.03)))
summary(obsMod)
```
...
```{r}
chao1Mod <- lm(S.chao1 ~ log(Size_Class) + I(log(Size_Class)^2) + I(log(Size_Class)^2) + Salinity + I(Salinity^2) + Oxygen + depth, data = diversityData %>% mutate(Oxygen = log10(Oxygen + 0.03)))
summary(chao1Mod)
```
As above but without latitude and depth
```{r}
chao1ModSimple <- lm(S.chao1 ~ log(Size_Class) + I(log(Size_Class)^2), data = diversityData %>% mutate(Oxygen = log10(Oxygen + 0.03)))
...
```{r}
shanMod <- lm(shannonH ~ log(Size_Class) + I(log(Size_Class)^2) + Salinity + I(Salinity^2) + Oxygen + depth, data = diversityData %>% mutate(Oxygen = log10(Oxygen + 0.03)))
```
...
```{r}
pielouMod <- lm(pielouJ ~ log(Size_Class) + I(log(Size_Class)^2) + Salinity + I(Salinity^2) + Oxygen+ depth, data = diversityData %>% mutate(Oxygen = log10(Oxygen + 0.03)))
summary(pielouMod)
```
...
```
```{r}
pielouMod2 <- lm(pielouJ2 ~ log(Size_Class) + I(log(Size_Class)^2) + Salinity + I(Salinity^2) + Oxygen, data = diversityDataWB %>% mutate(Oxygen = log(Oxygen + 0.03)))
summary(pielouMod2)
```
...
```{r}
breakLm2 <- lm(break_estimate ~ log(Size_Class) + I(log(Size_Class) ^2) + Salinity +  I(Salinity^2) + Oxygen ,data = diversityDataWB %>% mutate(Oxygen = log10(Oxygen + 0.03)))
breakLm2 %>% summary()
```
